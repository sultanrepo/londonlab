...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $labDb->execute("UPDATE doctors SET name=?,specialty=?,clinic_name=?,phone=?,email=?,address=?,commission_type=?,commission_value=? WHERE id=?",
            [$name,$spec,$clinic,$phone,$email,$addr,$ctype,$cval,$id]);
        labSetFlash('success','Doctor updated!');
        header('Location: '.LAB_APP_URL.'/modules/doctors/view.php?lab='.$slug.'&id='.$id); exit;
    }
    $doctor = array_merge($doctor, $_POST);
}
?>
<div class="page-header">
    <div><h2><i class="bi bi-pencil-square me-2" style="color:#7c3aed;"></i>Edit Doctor</h2><p><?= labClean($doctor['name']) ?></p></div>
    <a href="<?= LAB_APP_URL ?>/modules/doctors/view.php?lab=<?= $slug ?>&id=<?= $id ?>" class="btn btn-outline-secondary">
        ...
        <form method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-7">
                    ...
                    <input type="text" name="name" class="form-control" required value="<?= labClean($doctor['name']) ?>">
                </div>
                <div class="col-md-5">
                    <label class="form-label">Specialty</label>
                    <input type="text" name="specialty" class="form-control" value="<?= labClean($doctor['specialty']??'') ?>">
                </div>
                <div class="col-md-7">
                    <label class="form-label">Clinic / Hospital</label>
                    <input type="text" name="clinic_name" class="form-control" value="<?= labClean($doctor['clinic_name']??'') ?>">
                </div>
                <div class="col-md-5">
                    <label class="form-label">Phone</label>
                    <input type="text" name="phone" class="form-control" value="<?= labClean($doctor['phone']??'') ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Email</label>
                    <input type="email" name="email" class="form-control" value="<?= labClean($doctor['email']??'') ?>">
                </div>
                <div class="col-12">
                    <label class="form-label">Address</label>
                    <textarea name="address" class="form-control" rows="2"><?= labClean($doctor['address']??'') ?></textarea>
                </div>
                ...
                <div class="col-md-5">
                    ...
                    <select name="commission_type" id="ctype" class="form-select">
                        <option value="percentage" <?= $doctor['commission_type']==='percentage'?'selected':'' ?>>Percentage (%) of order</option>
                        <option value="fixed"      <?= $doctor['commission_type']==='fixed'?'selected':'' ?>>Fixed Amount (₹) per order</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Value</label>
                    <div class="input-group">
                        <span class="input-group-text" id="cpfx"><?= $doctor['commission_type']==='fixed'?'₹':'%' ?></span>
                        <input type="number" name="commission_value" id="cval" class="form-control" min="0" step="0.01" value="<?= labClean($doctor['commission_value']) ?>">
                    </div>
                </div>
                <div class="col-12">
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="<?= LAB_APP_URL ?>/modules/doctors/view.php?lab=<?= $slug ?>&id=<?= $id ?>" class="btn btn-outline-secondary">Cancel</a>
